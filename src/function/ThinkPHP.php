 ...
 * @return mixed
 */
function G($start,$end='',$dec=4, $MEMORY_LIMIT_ON=false) {
    static $_info       =   array();
    static $_mem        =   array();
 ...
 * @return string
 */
function xml_encode($data, $root='guangauns', $item='item', $attr='', $id='id', $encoding='utf-8') {
    if(is_array($attr)){
        $_attr = array();
 ...
 * @return mixed
 */
function session($name = '', $value = '', $config = [])
{
    // 默认设置
    $config = array(
    ...
    if (is_array($name)) {
        ...
        if (isset($name['prefix'])) {
            $config['SESSION_PREFIX'] = $name['prefix'];
        }
